          ...
          {loading ? (
            ...
            >
              ...
              {categories.map((category, catIndex) => {
                ...
                // Calculate indices for prev/current/next questions
                const currentQIndex = questionIndexForCategory;
                const prevQIndex = (currentQIndex - 1 + categoryQuestions.length) % categoryQuestions.length;
                const nextQIndex = (currentQIndex + 1) % categoryQuestions.length;
                
                const currentQ = categoryQuestions[currentQIndex];
                const prevQ = categoryQuestions[prevQIndex];
                const nextQ = categoryQuestions[nextQIndex];
                
                // Determine horizontal category positioning
                let categoryTransform = '';
                let categoryZIndex = 1;
                const isHorizontalDrag = isDragging && dragDirection === 'horizontal';
                const isHorizontalTransition = isTransitioning && (transitionDirection === 'left' || transitionDirection === 'right');
                ...
                if (isActiveCategory) {
                  if (isHorizontalDrag) {
                    ...
                    categoryTransform = `translateX(${dragOffsetX}px) scale(${scale}) rotate(${rotation}deg)`;
                  } else if (isHorizontalTransition && transitionDirection === 'left') {
                    categoryTransform = 'translateX(calc(-100% - 16px)) scale(0.8) rotate(-5deg)';
                  } else if (isHorizontalTransition && transitionDirection === 'right') {
                    categoryTransform = 'translateX(calc(100% + 16px)) scale(0.8) rotate(5deg)';
                  } else {
                    categoryTransform = 'translateX(0) scale(1) rotate(0deg)';
                  }
                  categoryZIndex = 3;
                  ...
                  if (isHorizontalDrag) {
                    const dragProgress = Math.abs(dragOffsetX) / 300;
                    const scale = Math.min(1, 0.8 + dragProgress * 0.2);
                    categoryTransform = `translateX(calc(-100% - 16px + ${dragOffsetX}px)) scale(${scale})`;
                  } else if (isHorizontalTransition && transitionDirection === 'right') {
                    categoryTransform = 'translateX(0) scale(1)';
                  } else {
                    categoryTransform = 'translateX(calc(-100% - 16px)) scale(0.8)';
                  }
                  categoryZIndex = 1;
                ...
                return (
                  ...
                  >
                    {/* Render vertical stack only for active category */}
                    {isActiveCategory && (
                      <>
                        {/* Previous Question - Hidden off-screen */}
                        {prevQ && (
                          <div
                            className="absolute inset-0 w-full h-full pointer-events-none"
                            style={{
                              transform: isDragging && dragDirection === 'vertical'
                                ? `translateY(calc(-70vh - 16px + ${dragOffsetY}px)) scale(${Math.min(1, 0.85 + Math.abs(dragOffsetY) / 300 * 0.15)})`
                        ...
                        {currentQ && (
                          <div
                            ...
                            style={{
                              transform: isDragging && dragDirection === 'vertical'
                                ? `translateY(${dragOffsetY}px) scale(${Math.max(0.85, 1 - Math.abs(dragOffsetY) / 300 * 0.15)})`
                        ...
                        {nextQ && (
                          <div
                            ...
                            style={{
                              transform: isDragging && dragDirection === 'vertical'
                                ? `translateY(calc(70vh + 16px + ${dragOffsetY}px)) scale(${Math.min(1, 0.85 + Math.abs(dragOffsetY) / 300 * 0.15)})`
                                : 'translateY(calc(70vh + 16px)) scale(1)',
                              zIndex: 1,
                              transition: isDragging ? 'none' : 'transform 0.3s ease-in-out'
                            }}
                          >
                    ...
                    {/* For non-active categories, only render current question */}
                    {!isActiveCategory && currentQ && (
